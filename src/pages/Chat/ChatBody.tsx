...
const ChatBody: React.FC<Props> = ({ userId }) => {
  ...
  // gql
  const { loading: message_loading, data: message_data } = useQuery(GET_USER_MESSAGE, {
    variables: { userId, pagination: { page, limit, sortOrder: SortOrder.Desc, sortBy: "createdAt" } },
    fetchPolicy: "network-only",
  });

  // set messages
  useEffect(() => {
    if (Array.isArray(message_data?.getUserMessage?.data)) {
      const m = [...(message_data?.getUserMessage?.data || [])];
      setMessages((pre) => [...pre, ...m.reverse()]);
    } else {
      setMessages([]);
    }
  }, [message_data]);
